...
<div id="content">
	...
	<!--Write code here!-->
		...
		<h2 class="heading">Section List</h2>
		<?php  
			include_once('../db_config.php');
			$slQuery = "SELECT pm.program,pm.department,pm.year,pm.semester,sm.section,sm.section_id"
					 . " FROM `sectionmaster` sm"
					 . " inner join program_master pm on sm.program_id=pm.program_id order by pm.program,pm.department,pm.year,sm.section";
			$result=mysql_query($slQuery);
			if($result) {
				$num=mysql_num_rows($result);
				if($num>0) {
					echo "<table width='650px' align='center' cellspacing='1' cellpadding='0' style='height:auto;overflow:auto'>";
					echo "<tr><th>S No</th>";
				...
				while($row=mysql_fetch_row($result)) {
					...
					$value = "<td>$i</td>"
							...
							."<td>$row[3]</td>"
							."<td>$row[4]</td>"
							."<td><a href=\"v_and_d_section.php?secid=$row[5]\">View</a></td>";
					echo "<tr class='$class'>$value</tr>";
				}
					echo "</tbody></table>";
				}
				else
					echo '<div class="msg">No Sections Found</div>';
			}
			else 
