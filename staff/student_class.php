...
class student_class 
{
	...
	*/
	...
	{
		require_once('../db_config.php');
		$selectquery = "SELECT regnumber,studentname,department,dateofbirth,yearofjoining FROM studentmaster "
						."WHERE regnumber = '$this->regNum'";
	...
	public function changePassword($oldPassword,$newPassword)
	{
		...
		if($this->password == $readPassword)
		{
			...
			$passQuery="UPDATE usermaster SET password = '$writePassword' WHERE username = '$this->regNum'";
	...
	{
		require_once('../db_config.php');
		$infoQuery = "SELECT attendance.date,attendance.period,remark.remark FROM attendance,remark "
					."WHERE attendance.relationid = '$relationId' AND remark.coursecode = '$courseCode' "
					. "and attendance.date = remark.date and attendance.period = remark.period order by date";
	...
	{
		$attendanceQuery = "SELECT attendance_data.coursecode,coursemaster.coursename,coursemaster.credits,attendance_data.num_present,attendance_data.num_working,attendance_data.post_date "
							."FROM attendance_data,coursemaster"
							." WHERE regnumber='$this->regNum'";
	...
	{	
		$ttQuery = "SELECT coursecode,section_master.section,period "
					."FROM relation,periodlist,section_master "
					."WHERE regnumber='{$this->regNum}' AND "
					."relation.relationid = periodlist.relationid AND "
					."relation.section_id = section_master.section_id";
	...
	{
		$ttDetailQuery="SELECT relation.coursecode,coursemaster.coursename,staffmaster.staffname,section_master.section "
						."FROM relation,coursemaster,staffmaster,section_master "
						...
						."relation.section = section_master.section_id";
