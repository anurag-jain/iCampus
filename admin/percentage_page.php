...
<div id="content">
	...
	<div id="studenthome" class="main">
	  <h2 class='heading'>Change Minimum Attendance Percentage</h2></br></br></br>
	<center>
    <?php
		require_once('../db_config.php');
		$query_percentage = "SELECT attrib_value from src_master where attribute='percentage'";
		$result_percentage = mysql_query($query_percentage);
		$row = mysql_fetch_row($result_percentage);
	?>    
	<p>Minimum % of attendance followed in SRC is
	  <input id="percentage_tb" name="percentage_tb" type="text" size="5" maxlength="3" value= "<?php echo $row[0] ?>"> 
	  %.
	</p>
    <p>
      <input type="button" onClick="percentage_ajax()" name="submit_per" id="submit_per" value="Change">
    </p>
    <p>&nbsp;</p>
	</center>
	</div>
